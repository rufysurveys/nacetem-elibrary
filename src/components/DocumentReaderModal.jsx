...
export default function DocumentReaderModal({ book, onClose, onAddNote, notes = [] }) {
  ...
  return (
    <div className="fixed inset-0 z-50 bg-slate-900/75 backdrop-blur-md flex items-center justify-center p-2 sm:p-4 md:p-6 overflow-hidden animate-in fade-in" role="dialog" aria-modal="true" aria-label={`Reading ${book.title}`}>
      {/* Portable Floating Window Container */}
      <div className={`w-full ${isFullscreen ? 'h-full max-w-none rounded-none' : 'max-w-5xl md:max-w-6xl h-[88vh] rounded-3xl'} bg-white border border-slate-200 shadow-2xl flex flex-col overflow-hidden relative transition-all duration-300`}>
        
        {/* Portable Reader Top Header Toolbar */}
        <header className="h-16 shrink-0 bg-white border-b border-slate-200 px-4 md:px-6 flex items-center justify-between gap-3 shadow-2xs">
          <div className="flex items-center gap-3 min-w-0">
            ...
            <div className="min-w-0">
              <h2 className="font-extrabold text-sm md:text-base text-slate-900 truncate max-w-md">{book.title}</h2>
          ...
          <div className="flex items-center gap-1.5 shrink-0">
            {fileUrl && (
              <span className="hidden xl:flex items-center gap-1 text-[11px] font-extrabold text-emerald-800 bg-emerald-50 border border-emerald-300 px-2.5 py-1 rounded-lg">
                <FileCheck2 className="w-4 h-4 text-emerald-700" /> Preserved PDF
              </span>
            )}
        ...
        {/* Portable Reader Viewport & Sidebar */}
        <div className="flex flex-1 min-h-0 relative">
          <main className="flex-1 min-w-0 bg-slate-900 flex flex-col relative">
            {pdfUrl ? (
              ...
              <div className="h-full overflow-y-auto p-4 md:p-8 bg-slate-100">
                <article className="max-w-3xl mx-auto bg-white text-slate-900 p-6 md:p-12 shadow-md rounded-2xl space-y-6 border border-slate-200">
                  <div className="border-b border-slate-200 pb-4 space-y-2">
                    ...
                    <h1 className="text-xl md:text-2xl font-black text-slate-900 leading-tight">{book.title}</h1>
                    {book.subtitle && <p className="text-xs font-semibold text-slate-600 italic">{book.subtitle}</p>}
                  ...
                  <div className="p-4 bg-emerald-50/70 rounded-2xl border border-emerald-200 space-y-2">
                    ...
                    <p className="text-xs md:text-sm text-slate-800 leading-relaxed font-serif">{book.abstract}</p>
                  </div>
                  ...
                  <div className="space-y-6 pt-2">
                    {(book.fullText || [{ sectionTitle: 'Full Text', content: book.abstract }]).map((section, index) => (
                      <section key={index} className="space-y-2">
                        <h2 className="text-base font-black text-slate-900 border-b border-slate-200 pb-1.5">
                          {section.sectionTitle}
                        </h2>
                        <p className="whitespace-pre-line leading-relaxed text-xs md:text-sm text-slate-800 font-serif">
                          {section.content}
                        </p>
                      </section>
                    ))}
          ...
          {(showCitation || showNotes) && (
            <aside className="w-80 shrink-0 bg-white text-slate-900 border-l border-slate-200 overflow-y-auto p-5 shadow-xl animate-in slide-in-from-right z-10">
              {showCitation && (
                <section className="space-y-4">
                  ...
                  <div className="p-3 bg-slate-50 border border-slate-200 rounded-xl font-mono text-xs text-slate-800 leading-relaxed break-words">
                    {citation}
                  </div>
